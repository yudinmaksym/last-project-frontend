...
let overrides = (typeof window === 'undefined') ? {}
  : (window.ShardsDashboards && window.ShardsDashboards.colors)
  ...
  for (var colorName in overrides) {
    if (overrides.hasOwnProperty(colorName)) {
      if (!/^#([A-Fa-f0-9]{3}$)|([A-Fa-f0-9]{6}$)/.test(overrides[colorName]))
        throw new Error('Please provide a hexadecimal color value if you are trying'
        +'to override the Shards Dashboards colors.')
...
const charts = {
  actual: new Color('#003f5c'),//new Color('yellow'),
  baseline: new Color('#a05195'),//new Color('red'),
  adjustedbaseline: new Color('#d45087'),//new Color('green'),
  forecast: new Color('#ffa600'),//new Color('orange'),
}
